...
public static class UserMappings
{
    ...
    public static UserProfileDto ToProfileDto(this User? user) =>
      new(
          string.IsNullOrWhiteSpace(user?.UserName) ? DefaultConstants.User.DefaultUserName : user!.UserName,
          string.IsNullOrWhiteSpace(user?.FirstName) ? DefaultConstants.User.DefaultFirstName : user!.FirstName,
          user?.LastName ?? string.Empty,
          string.IsNullOrWhiteSpace(user?.Email) ? $"{DefaultConstants.User.DefaultUserName}@noemail.com" : user!.Email,
    ...
    public static UserListDto ToUserListDto(this User? user) =>
        new(string.IsNullOrEmpty(user?.UserName) ? DefaultConstants.User.DefaultUserName : user!.UserName,
            user?.AvatarId ?? Guid.Empty);
}
